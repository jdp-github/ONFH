...
Page({
  ...
  onLoad: function(options) {
    ...
    this.setData({
      ...
      times: options.times,
      isLeftClose: options.left_close ? options.left_close == 1 : false,
      isRightClose: options.right_close ? options.right_close == 1 : false
    });
    if (!this.data.isCreateCase) {
      this.requestCaseInfo(options.times)
    } else {
      // 左侧
      if (this.data.isLeftClose) {
        this.closeLeft()
      }
      // 右侧
      if (this.data.isRightClose) {
        this.closeRight()
      }
    }
    // else {
  ...
  closeRight() {
    this.setData({
      timesDisabled_R: true,
      sfDateDisabled_R: true,
      sfqkDisabled_R: true,
      zjbcDisabled_R: true,
      sfzcDisabled_R: true,
      ggfaDisabled_R: true,
      arcoDisabled_R: true,
      harrisDisabled_R: true,
      vasDisabled_R: true,
      xleftDisabled_R: true,
      ctLeftDisabled_R: true,
      mriLeftDisabled_R: true,
      bczlDisabled_R: true,
      jtzlDisabled_R: true,
      beizhuDisabled_R: true
    })
  },

  requestCaseInfo(times) {
    let that = this;
    wx.request({
      url: constant.basePath,
      data: {
      ...
      success(res) {
        console.log("Followup.GetInfo:" + JSON.stringify(res))
        that.hideLoading();
  ...
  initViewByData(followupInfo) {
    this.setData({
      followupInfo: followupInfo
    });

    if (followupInfo.info.length > 1) {
      // 左侧
      this.setLeftData(followupInfo.info[0])
      // 右侧
      this.setRightData(followupInfo.info[1])
    } else if (followupInfo.info.length == 1) {
      let info = followupInfo.info[0]
      if (info.side == 1) { // 左侧
        this.setLeftData(info)
        this.closeRight()
        this.setData({
          isRightClose: true
        })
      } else { // 右侧
        this.setRightData(info)
        this.closeLeft()
        this.setData({
          isLeftClose: true
        })
  ...
  setLeftData(leftInfo) {
    this.setData({
      times: leftInfo.times,
      sfDate: util.formatTime(leftInfo.sf_date, 'Y-M-D'),
      sfDateDisabled: this.getNumDisable(leftInfo.sf_date),
      szscValue: leftInfo.sf_jlszsc,
      sfqkIndex: leftInfo.sf_zt,
      sfqkDisabled: this.getNumDisable(leftInfo.sf_zt),
      zjbcIndex: leftInfo.sf_zj,
      zjbcDisabled: this.getNumDisable(leftInfo.sf_zj),
      sfzcIndex: leftInfo.sf_scfa,
      sfzcDisabled: this.getNumDisable(leftInfo.sf_scfa),
      ggfaValue: leftInfo.sf_ggfa,
      ggfaDisabled: this.getValueDisable(leftInfo.sf_ggfa),
      arcoIndex: leftInfo.arco,
      arcoDisabled: this.getNumDisable(leftInfo.arco),
      harris: this.getDefaultNum(leftInfo.harris),
      harrisImgArr: this.makeImgArr(leftInfo.harris_pics),
      harrisDisabled: leftInfo.harris == 0,
      vasIndex: leftInfo.vas,
      vasImgArr: this.makeImgArr(leftInfo.vas_pics),
      vasDisabled: this.getNumDisable(leftInfo.vas),
      xleftIndex: leftInfo.x_ispai,
      xleftImgArr: this.makeImgArr(leftInfo.x_pic),
      xleftDisabled: this.getNumDisable(leftInfo.x_ispai),
      ctLeftIndex: leftInfo.ct_ispai,
      ctLeftImgArr: this.makeImgArr(leftInfo.ct_pic),
      ctLeftDisabled: this.getNumDisable(leftInfo.ct_ispai),
      mriLeftIndex: leftInfo.mri_ispai,
      mriLeftImgArr: this.makeImgArr(leftInfo.mri_pic),
      mriLeftDisabled: this.getNumDisable(leftInfo.mri_ispai),
      bczlIndex: leftInfo.bczl,
      bczlDisabled: this.getNumDisable(leftInfo.bczl),
      jtzl: leftInfo.jtzl,
      jtzlDisabled: this.getValueDisable(leftInfo.jtzl),
      beizhu: leftInfo.beizhu,
      beizhuDisabled: this.getValueDisable(leftInfo.beizhu)
    })
  },

  setRightData(rightInfo) {
    this.setData({
      sfDate_R: util.formatTime(rightInfo.sf_date, 'Y-M-D'),
      sfDateDisabled_R: this.getNumDisable(rightInfo.sf_date),
      szscValue_R: rightInfo.sf_jlszsc,
      sfqkIndex_R: rightInfo.sf_zt,
      sfqkDisabled_R: this.getNumDisable(rightInfo.sf_zt),
      zjbcIndex_R: rightInfo.sf_zj,
      zjbcDisabled_R: this.getNumDisable(rightInfo.sf_zj),
      sfzcIndex_R: rightInfo.sf_scfa,
      sfzcDisabled_R: this.getNumDisable(rightInfo.sf_scfa),
      ggfaValue_R: rightInfo.sf_ggfa,
      ggfaDisabled_R: this.getValueDisable(rightInfo.sf_ggfa),
      arcoIndex_R: rightInfo.arco,
      arcoDisabled_R: this.getNumDisable(rightInfo.arco),
      harris_R: this.getDefaultNum(rightInfo.harris),
      harrisImgArr_R: this.makeImgArr(rightInfo.harris_pics),
      harrisDisabled_R: rightInfo.harris == 0,
      vasIndex_R: rightInfo.vas,
      vasImgArr_R: this.makeImgArr(rightInfo.vas_pics),
      vasDisabled_R: this.getNumDisable(rightInfo.vas),
      xleftIndex_R: rightInfo.x_ispai,
      xleftImgArr_R: this.makeImgArr(rightInfo.x_pic),
      xleftDisabled_R: this.getNumDisable(rightInfo.x_ispai),
      ctLeftIndex_R: rightInfo.ct_ispai,
      ctLeftImgArr_R: this.makeImgArr(rightInfo.ct_pic),
      ctLeftDisabled_R: this.getNumDisable(rightInfo.ct_ispai),
      mriLeftIndex_R: rightInfo.mri_ispai,
      mriLeftImgArr_R: this.makeImgArr(rightInfo.mri_pic),
      mriLeftDisabled_R: this.getNumDisable(rightInfo.mri_ispai),
      bczlIndex_R: rightInfo.bczl,
      bczlDisabled_R: this.getNumDisable(rightInfo.bczl),
      jtzl_R: rightInfo.jtzl,
      jtzlDisabled_R: this.getValueDisable(rightInfo.jtzl),
      beizhu_R: rightInfo.beizhu,
      beizhuDisabled_R: this.getValueDisable(rightInfo.beizhu)
    })
  },
  ...
  editLeft() {
    ...
    wx.request({
      url: constant.basePath,
      data: {
        service: 'Followup.EditLeftRight',
        openid: app.globalData.openid,
        json_data: that.makeLeftData(),
        followup_id: that.data.followupInfo.info[0].id
      },
      header: {
        'content-type': 'application/json'
      },
  ...
  makeLeftData() {
    ...
    var jsonData = {
      ...
      // Harris
      harris: that.data.harris ? that.data.harris : 0,
  ...
  editRight() {
    let that = this;
    let followupId = ''
    if (that.data.isLeftClose) {
      followupId = that.data.followupInfo.info[0].id
    } else {
      followupId = that.data.followupInfo.info[1].id
    }
    wx.request({
      url: constant.basePath,
      data: {
  ...
  makeRightData() {
    ...
    var jsonData = {
      ...
      // Harris
      harris: that.data.harris_R ? that.data.harris_R : 0,
